...
- [x] **5.1** CI: добавить `rust-cache` в `native` job
- [x] **5.2** ~~Подумать про `autocfg` вместо ручного probe в `build.rs`~~ — текущий probe простой и работает, `autocfg` добавит лишнюю зависимость без выгоды
- [x] **5.3** Удалить Windows из `release.yml` (неподдерживаемая платформа)
